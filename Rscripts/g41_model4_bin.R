...
# output_files <- c(
#   paste0(outfol,"models/binaryAgriculture/model4R_bin.rds"),
#   paste0(outfol,"models/binaryAgriculture/model40R_bin.rds"),
#   paste0(outfol,"models/binaryAgriculture/model42R_bin.rds"),
# paste0(outfol,"models/binaryAgriculture/model42_bin_summary.txt")
#   )
# if(all(file.exists(output_files))){
#   cat(paste0("The output files for this model run already exists.\n", 
#              output_files), ".\n")
# }else{
#   script_name <- "g41_model4.R"
#   start_time <- Sys.time()
...
#############
#### model ## political complexity ~ alcohol + agriculture (binary) + space + phylogeny
#############
...
summary(model4_bin)
 save(model4_bin, file=paste0(outfol,"models/binaryAgriculture/model4R_bin.rds"))

warnings_model40_bin <- NULL
...
model40_bin<- brm (m40 ,data=thedata, cores=4, chains=2, data2 = list(phylo_covar_mat = phylo_covar_mat, spatial_covar_mat = spatial_covar_mat),
                  iter=4000,control = list(adapt_delta = 0.99), prior=prior) 
warnings_model40_bin <- warnings()

#n_re_runs <- 1
#while(length(warnings_model40_bin) != 0){
#  warnings_model40_bin <- NULL
#  cat(paste0("Total re-runs of model40_bin", n_re_runs, ".\n"))
#  n_re_runs <- n_re_runs + 1   
#    model40_bin<- brm (m40 ,data=thedata, cores=4, chains=2, data2 = list(phylo_covar_mat = phylo_covar_mat, spatial_covar_mat = spatial_covar_mat),
...
save(model40_bin, file=paste0(outfol,"models/binaryAgriculture/model40R_bin.rds"))
conditional_effects(model40_bin, "Alcohol", categorical = TRUE)
capture.output(summary(model40_bin), file=paste0(outfol,"models/binaryAgriculture/model40_bin_summary.txt"))
...
# # EXPORT
 capture.output(summary(model4_bin), file=paste0(outfol,"models/binaryAgriculture/model4_bin_summary.txt"))
...
summary(model42_bin)
 ...
 save(model42_bin, file=paste0(outfol,"models/binaryAgriculture/model42R_bin.rds"))
 capture.output(summary(model42_bin), file=paste0(outfol,"models/binaryAgriculture/model42_bin_summary.txt"))

load(file=paste0(outfol,"models/binaryAgriculture/model42R_bin.rds"))
